...
struct Settings {
  ...
  uint8_t alertTune; // 0..4 — index into MELODIES table for prompt-arrival sound
};
...
inline void settingsLoad() {
  ...
  _settings.alertTune = _prefs.getUChar("s_atune", 0);
  if (_settings.alertTune > 4) _settings.alertTune = 0;
  _prefs.end();
}
...
inline void settingsSave() {
  ...
  _prefs.putUChar("s_atune", _settings.alertTune);
  _prefs.end();
}
